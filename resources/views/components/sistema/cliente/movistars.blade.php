...
<x-sistema.card class="m-2 mt-0">
    ...
    <div class="form-group w-full max-w-xs mt-2">
        <div class="relative mb-2">
            <select id="estadowick_id" class="block px-2.5 pb-1.5 pt-1.5 w-full text-xs text-[#333333] bg-[#F1F5F9] rounded-lg border-1 border-[#D1D5DB] appearance-none peer" @php echo ($movistar != '' ? 'disabled' : '') @endphp>                @foreach ($estadowicks as $value)
    ...
    @if ($config['datosAdicionales']['estadoDito'])
        <div class="relative mb-1">
            <select id="estadodito_id" class="block px-2.5 pb-1.5 pt-1.5 w-full text-xs text-[#333333] bg-[#F1F5F9] rounded-lg border-1 border-[#D1D5DB] appearance-none peer" @php echo ($movistar != '' ? 'disabled' : '') @endphp>
    ...
    @if ($config['datosAdicionales']['tipoCliente'])
        <div class="relative mb-3">
            <select id="clientetipo_id" class="block px-2.5 pb-1.5 pt-1.5 w-full text-xs text-[#333333] bg-[#F1F5F9] rounded-lg border-1 border-[#D1D5DB] appearance-none peer" @php echo ($movistar != '' ? 'disabled' : '') @endphp>
    ...
    @if ($config['datosAdicionales']['ejecutivoSalesforce'])
        <div class="relative mb-3">
            ...
            <label for="ejecutivo_salesforce" class="absolute text-sm text-gray-600 duration-300 transform -translate-y-4 scale-75 top-2 left-0 z-10 origin-[0] px-0.5 py-0.5 peer-focus:text-blue-600 peer-focus:text-sm peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-100 peer-focus:-translate-y-4 bg-[#F1F5F9]">
    ...
    @if ($config['datosAdicionales']['agencia'])
        <div class="relative mb-1">
            <select id="agencia_id" class="block px-2.5 pb-1.5 pt-1.5 w-full text-xs text-[#333333] bg-[#F1F5F9] rounded-lg border-1 border-[#D1D5DB] appearance-none peer" @php echo ($movistar != '' ? 'disabled' : '') @endphp>
